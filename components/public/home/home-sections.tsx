      ...
      <div className="container mx-auto px-4">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            />
            <div className="relative bg-[#f0f4ff] rounded-2xl p-8">
              <Quote className="absolute top-4 left-4 w-10 h-10 text-[#1a3c6e]/10" />
              ...
              <div className="flex items-center gap-4 mt-6 pt-6 border-t border-[#1a3c6e]/10">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#1a3c6e] to-[#0d1f3c] flex items-center justify-center text-white font-black text-xl">P</div>
                <div>
                  <p className="font-bold text-[#1a3c6e]">The Principal</p>
            ...
            {[
              ...
              return (
                <div key={item.label} className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow text-center">
                  ...
                  <p className="text-xs text-[#c8a951] font-semibold">{item.sub}</p>
          ...
          {features.map((f) => {
            ...
            return (
              <div key={f.title} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group">
                ...
                <h3 className="font-bold text-[#1a3c6e] text-lg mb-2 group-hover:text-[#c8a951] transition-colors">{f.title}</h3>
...
export function Facilities() {
  ...
  return (
    <section className="py-20 bg-gradient-to-br from-[#1a3c6e] to-[#0d1f3c] text-white relative overflow-hidden">
          ...
          {items.map((item) => {
            ...
            return (
              <div key={item.label} className="text-center group">
                <div className="w-14 h-14 mx-auto rounded-2xl bg-white/10 border border-white/20 group-hover:bg-[#c8a951] group-hover:border-[#c8a951] transition-all duration-300 flex items-center justify-center mb-3">
              ...
              {news.length === 0 ? (
                ...
                news.slice(0, 3).map((item) => (
                  <div key={item.id} className="flex gap-4 p-4 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all group">
                    <div className="w-20 h-20 rounded-xl bg-[#f0f4ff] flex-shrink-0 overflow-hidden">
                      {item.featuredImage ? (
                    ...
                    <div>
                      <p className="font-bold text-[#1a3c6e] text-sm group-hover:text-[#c8a951] transition-colors line-clamp-2">{item.title}</p>
                ...
                events.slice(0, 4).map((event) => {
                  ...
                  return (
                    <div key={event.id} className="flex gap-4 p-4 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex-shrink-0 w-14 text-center bg-[#1a3c6e] rounded-xl px-2 py-3">
                        <p className="text-[#c8a951] font-black text-xl leading-none">{d.getDate()}</p>
                      ...
                      <div className="flex-1">
                        <p className="font-bold text-[#1a3c6e] text-sm">{event.title}</p>
  ...
  return (
    <section className="py-20 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.map((t) => (
            <div key={t.id} className="bg-[#f8f9ff] rounded-2xl p-6 border border-gray-100 hover:shadow-lg transition-shadow relative">
              <Quote className="absolute top-5 right-5 w-8 h-8 text-[#1a3c6e]/10" />
              <div className="flex gap-0.5 mb-4">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className={cn("w-4 h-4", i < t.rating ? "text-[#c8a951] fill-[#c8a951]" : "text-gray-300")} />
                ))}
              </div>
              <p className="text-gray-700 text-sm leading-relaxed mb-6 italic">"{t.content}"</p>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#1a3c6e] to-[#0d1f3c] flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                  {t.name.charAt(0)}
                </div>
                <div>
...
export function CTABanner() {
  return (
    <section className="py-20 bg-gradient-to-r from-[#c8a951] to-[#e8c97a] relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative">
        <div className="text-center max-w-2xl mx-auto">
          <h2 className="text-3xl sm:text-4xl font-black text-[#1a3c6e] mb-4">Want to Know More About Our School?</h2>
          <p className="text-[#1a3c6e]/80 text-base mb-8">We'd love to show you what makes Guru Gorakshnath Gyanasthali a special place for your child. Reach out anytime.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/contact">
              <Button variant="default" size="lg" className="bg-[#1a3c6e] text-white hover:bg-[#0d1f3c] shadow-lg">
                Get in Touch <ArrowRight className="w-5 h-5" />
              </Button>
            </Link>
            <Link href="/about">
              <Button variant="outline" size="lg" className="border-2 border-[#1a3c6e] text-[#1a3c6e] hover:bg-[#1a3c6e] hover:text-white">
                Learn About Us
              </Button>
            </Link>
          </div>
          <div className="flex items-center justify-center gap-6 mt-8 text-sm text-[#1a3c6e]/70 flex-wrap">
            {["Experienced Faculty", "CBSE Affiliated", "Safe & Nurturing Campus"].map((item) => (
              <span key={item} className="flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4 text-[#1a3c6e]" />
                {item}
              </span>
            ))}
          </div>
          ...
          <div className="flex items-center justify-center gap-4 mt-8">
            <a href="https://www.facebook.com/61555972687989/" target="_blank" rel="noopener noreferrer" className="w-11 h-11 rounded-xl bg-[#1a3c6e] hover:bg-[#0d1f3c] flex items-center justify-center text-white transition-colors shadow-md" aria-label="Facebook">
