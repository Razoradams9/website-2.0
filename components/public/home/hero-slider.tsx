...
const slides = [
  {
    ...
    gradient: "from-[#0d1f3c] via-[#1a3c6e] to-[#1a3c6e]",
  },
  {
    id: 2,
    ...
    gradient: "from-[#1a3c6e] via-[#0d1f3c] to-[#0a1628]",
  },
  {
    id: 3,
    ...
    gradient: "from-[#0a1628] via-[#1a3c6e] to-[#0d2b4e]",
  },
]
  ...
  return (
    <section className="relative min-h-[90vh] flex flex-col overflow-hidden">
          ...
          <div className="max-w-3xl">
            ...
            {current === 0 && (
              <div className="mb-8 animate-fade-in">
                <img src="/images/logo.png" alt="Guru Gorakshnath Gyanasthali" className="w-28 h-28 sm:w-36 sm:h-36 object-contain drop-shadow-2xl" />
              </div>
            )}

            <div
              key={`badge-${current}`}
              className="inline-flex items-center gap-2 bg-[#c8a951]/20 border border-[#c8a951]/30 text-[#c8a951] rounded-full px-4 py-1.5 text-sm font-semibold mb-6 animate-fade-in"
            >
            ...
            >
              {slide.title}{" "}
              <span className="text-[#c8a951] block sm:inline">{slide.titleAccent}</span>
            </h1>
            ...
            >
              <Link href={slide.cta.href}>
                <Button variant="gold" size="lg" className="text-base px-8 shadow-lg shadow-[#c8a951]/20">
                  {slide.cta.label}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 divide-x divide-white/10">
            {stats.map((stat) => (
              <div key={stat.label} className="text-center py-5 px-4">
                <p className="text-2xl lg:text-3xl font-black text-[#c8a951]">{stat.value}</p>
      ...
      <div className="absolute bottom-20 left-1/2 -translate-x-1/2 flex gap-2">
        {slides.map((_, i) => (
          <button key={i} onClick={() => goTo(i)} className={cn("transition-all duration-300 rounded-full", i === current ? "w-8 h-2 bg-[#c8a951]" : "w-2 h-2 bg-white/40 hover:bg-white/70")} />
        ))}
      </div>
    </section>
  )
